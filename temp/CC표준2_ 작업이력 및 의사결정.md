# **CC표준2: CC 작업이력·의사결정 (v6.0 / 2026-09-03)**

**기준일:** 2026-09-03
**이전 버전:** v5.0 (2026-09-03, 결절제거 데스크탑·모바일 라이브 배포)
**변경 사유:** 결절제거 llms .md·sitemap 25개(과제 8 완료), 영문 사이트 결절제거 반영(과제 9 완료), 영문 가격표 price_en.webp 교체·배포(과제 6 완료) — 완료 이력 3건 추가
...
### **2026-09-03: 결절제거 llms .md 생성 및 sitemap 등록 (과제 8 완료)**

다른 8개 시술과 동일하게 nodule-removal-injection/index.html.md를 생성하고 sitemap.xml에 .md URL을 추가했습니다(24→25개). AI 크롤러 대응 일관성 확보. dev→main 병합 라이브 배포 완료.

### **2026-09-03: 영문 사이트(en/) 결절제거 반영 (과제 9 완료)**

en/index.html에 결절제거 카드 추가, en/data/services/nodule.json 생성, en/js/service-panel.js serviceIdMap에 service9 엔트리 추가, 케이스 이미지 2개(nodule_01·02) 복사. en/data/services-index.json은 영문 사이트가 로드하지 않는 미사용 참조 데이터로 확인되어 등록을 생략했습니다(의사결정).

### **2026-09-03: 영문 가격표 이미지 영문화 (과제 6 완료)**

file.cclinic.kr/images/price_en.webp를 영문 가격표 이미지로 교체·배포했습니다(146,258 B). 기존 price_en.png는 삭제. Cloudflare 캐시 purge로 즉시 반영 확인. 단일 소스(single source of truth) 구조로 전 채널 동시 반영.
...
### **2026-09-02: 결절제거주사 페이지 신규 구축 및 데스크탑 네비게이션 개편 (2026-09-03 배포 완료)**

9번째 시술 페이지인 결절제거주사(nodule-removal-injection)를 파워라인 페이지 형식(FAQ 아코디언·아이콘)을 템플릿으로 신규 구축했습니다(index.html 881줄, 커밋 033b91f). js/common.js 모바일 리다이렉트 정규식에 nodule-removal-injection 경로를 추가했고(커밋 5f30200), 케이스 이미지 2장(images/cases/nodule_01.jpg·nodule_02.jpg)을 업로드했습니다(커밋 a2739f6).

데스크탑 전 페이지 상단 네비게이션과 FAQ 팝업(faq-item-desktop)에 결절제거 링크를 추가했습니다(11파일, 커밋 c8a700b). 이후 상단 메뉴에 "바디케어" 드롭다운을 신설(파워라인 다음 위치)하여 셀룰라이트·튼살을 하위로 재배치하고, 링크 추가 과정에서 발생한 유착제거·결절제거 두 줄 겹침을 각각 독립 nav-item으로 분리했습니다(12파일, 커밋 57d0273). 최종 데스크탑 메뉴 순서는 지방이식제거 → 유착제거 → 결절제거 → 눈두덩이주사 → 윤곽주사 → 브이라인 → 파워라인 → 바디케어(셀룰라이트·튼살) → 시술후기 → 언론보도 → 상담·문의입니다.

모든 작업은 dev 브랜치에서 진행 후 2026-09-03 dev→main 병합으로 라이브 배포되었습니다. 이후 케이스 이미지 2장을 교체하여 데스크탑·모바일 동일 적용했습니다. 상세 내역은 temp/작업로그_2026-09-02_결절제거.md 참조.

### **2026-09-03: 결절제거주사 라이브 배포 및 모바일 반영·이미지 교체**
모바일 반영: m/index.html에 시술 카드(유착제거 뒤)·상담 폼 옵션·FAQ 팝업을 추가하고, service-panel.js serviceIdMap에 service9→nodule 매핑, m/data/services/nodule.json 및 services-index.json 등록을 완료했습니다. sitemap.xml에 결절제거 HTML URL 추가(23→24개). dev→main 병합(PR #1, 병합커밋 f5baf2c)으로 라이브 배포 후, 케이스 이미지 2장을 새 이미지로 교체(커밋 cb92057)하여 데스크탑·모바일 동일 적용하고 dev·main을 동기화했습니다.
...
| 완료 | sitemap.xml 갱신 | 2026-09-03 결절제거 URL 추가(24개) 및 배포 |
| 보류 | 영문 가격표 이미지(price\_en.webp) 영문 변경 | 현재 한글로 되어 있으며 추후 영문 변경 예정 |
| 보류 | llms.txt 영문 사이트 정보 보강 | 현재 엔트리만 추가된 상태, 상세 설명 추가 검토 |
| 완료 | 결절제거주사 페이지 (데스크탑) | 2026-09-03 라이브 배포 완료 |
| 완료 | 결절제거 모바일 페이지 및 main 배포 | 2026-09-03 모바일 반영·sitemap 등록·dev→main 병합 배포 완료 |
